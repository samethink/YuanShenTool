...
from src.utils.support import config, logger, SYSTEM_NAME
...
class Web:
    ...
    def get_inventory(self, share_code):
        ...
        logger.debug('%s <- %s', url, ret.text[:100])
        return ret
...
class FetchInv:
    ...
    def download_inventory(self, share_code):
        ...
        elif not self.web.cookie or len(self.web.cookie) < 32:
            return False, '<set_cookie>'

        response = self.web.get_inventory(share_code)
        try:
            resp = response.json()
            # resp = load_json_file('debug/response.json')
            if resp['data']:
                info_list = resp['data']['list'] + resp['data']['not_calc_list']
                ...
                return False, 'cookie已失效：' + resp['message']
            else:
                return False, '失败：' + resp['message']
        except requests.JSONDecodeError:
            return False, '响应结果解析失败'

    def save_inventory_as_xlsx(self, data, filename):
        """保存文件
    ...
    def get_item_icon(self, item):
        ...
        if str(item['id']) not in self.__item_ids:
            with open(icon_path, 'wb') as fp:
                fp.write(requests.get(item['icon_url'], headers={'User-Agent': UA}).content)
